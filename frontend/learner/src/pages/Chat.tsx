  ...
  const sendMessage = useCallback(async (content: string) => {
    ...
    try {
      ...
      es.addEventListener('error', () => {
        setMessages((prev) =>
          prev.map((m) => m.id === aiId ? { ...m, content: m.content || 'Sorry, something went wrong.', isStreaming: false } : m),
        )
        setIsStreaming(false)
        es.close()
      })
      ...
      setMessages((prev) =>
        prev.map((m) => m.id === aiId ? { ...m, content: 'Could not reach the AI Tutor. Is the companion service running?', isStreaming: false } : m),
      )
      setIsStreaming(false)
    }
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
        ...
        {messages.map((msg) => (
          <div
            key={msg.id}
            className={`flex flex-col gap-1 animate-fade-in ${msg.role === 'learner' ? 'items-end' : 'items-start'}`}
          >
            <div
